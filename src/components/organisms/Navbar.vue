<template>
	...
	>
		...
		:class="{
			open,
			hidden: child || !mini,
			'justify-center': ['center', 'center-right'].includes(align),
	...
	<ul
		...
		:class="{
			...
			root: !child,
			flex: !mini,
			'align-right': ['right', 'center-right'].includes(align),
			right: align === 'right',
			left: !['right', 'center-right', 'center'].includes(align)
		}"
	>
		<li
			v-for="item in items"
		...
		>
			...
			/> <svg
				...
				v-if="['right', 'center-right'].includes(align)"
			>
				<path
					fill-rule="evenodd"
			...
			/> <router-link
				...
				:to="item.route"
				:class="{
					'text-right': ['center-right', 'right'].includes(align)
				}"
...
<script>
	export default {
		...
		methods: {
			isMini() {
				this.mini = window.innerWidth < this.breakpoint;
			},
			setItems() {
